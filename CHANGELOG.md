...
- Specialist consultation step in brainstorming. Before proposing approaches, the brainstormer invokes the specialist skill(s) matching each language or domain in scope.
- Specialist routing in subagent-driven-development. The implementer subagent is now domain-routed (`python` → `python-pro`, `rust` → `rust-pro`, etc.) with `general-purpose` as the fallback.
