...
How to modify your fork of atomic-superpowers to fit your stack.
...
Three places reference agents and skills by name. When you add, swap, or remove, those are the files to update.
...
| Domain vocabulary | `skills/atomic-issues/SKILL.md`, "Single language or domain" section | Lists the domains decomposition recognizes |

If your change adds or removes a domain (a language or platform), update all three. If it's a new skill within an existing domain, only the brainstorming routing needs an update.
...
3. Update three routing tables:
   - `skills/atomic-issues/SKILL.md` - add `php` to the bundled domains list
...
The atomic-issues skill and subagent-driven-development don't need updates - they route on domain, not on individual skills.
...
4. Update routing tables:
   - `skills/atomic-issues/SKILL.md` - drop `rust` from the bundled domains list
...
Mechanically the same as "add a specialist agent for a new language." The atomic-issues skill currently lists only languages in its examples; if your domain isn't language-shaped, update the examples there too so the brainstormer's decomposition step recognizes the new category.
