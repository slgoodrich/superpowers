...
run_claude_turn "01-idea" "I want to build a small Python CLI tool that reads a CSV file and inserts each row into a SQLite database table. Take the CSV path and target table name as command-line arguments. Use only the Python 3.12 standard library (csv, sqlite3, argparse). I want this decomposed into atomic, shippable issues per the atomic-superpowers workflow. Project context: empty directory with a README and a sample.csv. No need for the visual companion. Please go through the brainstorming, decomposition, and execution flow." "new" 300 8

run_claude_turn "02-clarify" "Schema: just one table with a configurable name. Columns derived from the CSV header row, all stored as TEXT for simplicity. Skip duplicate detection - re-running should append. Tests with pytest. Use your best judgment for any other details, prefer simple over clever, follow Python and SQL idioms." "continue" 300 8

run_claude_turn "03-approve-design" "The design and approach look good. Proceed: write the spec doc, do the spec self-review, decompose into atomic issues, and append the issue list to the spec." "continue" 300 8

run_claude_turn "04-approve-spec" "Spec and decomposition look good. Proceed with the first atomic issue: write its plan, then execute via subagent-driven-development." "continue" 300 8
...
if [[ "$SKIP_EXECUTION" == "false" ]]; then
    ...
    run_claude_turn "05-execute-issue-1" "Continue. Execute the first issue's plan via subagent-driven-development through all reviews until the issue's PR-ready state. Then move to the next issue: write its plan, execute it, all the way through. Continue until all decomposed atomic issues are complete." "continue" 1800 50

    # If turn 05 hit max-turns or timeout before finishing all issues,
    # nudge it forward.
    run_claude_turn "06-continue" "Continue with the remaining issues. If the previous one is complete, write the plan for the next atomic issue and execute it." "continue" 1800 50

    run_claude_turn "07-continue" "Continue with any remaining issues. If everything is done, summarize what was built." "continue" 1800 50
fi
